...
CREATE TABLE
    IF NOT EXISTS orders (
        ...
        order_id VARCHAR(36) UNIQUE NOT NULL COMMENT 'Unique order identifier',
        ...
        status ENUM (
            ...
            'CANCELLED',
            'FAILED'
        ) DEFAULT 'PENDING' COMMENT 'Order status in saga',
        saga_id VARCHAR(36) COMMENT 'Associated saga ID for tracking',
...
CREATE TABLE
    IF NOT EXISTS saga_log (
        saga_id VARCHAR(36) PRIMARY KEY COMMENT 'Saga identifier',
        current_step VARCHAR(50) COMMENT 'Current step in saga execution',
        status ENUM ('STARTED', 'COMPLETED', 'COMPENSATED', 'FAILED') DEFAULT 'STARTED' COMMENT 'Saga status',
